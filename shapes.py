...
SVG.py - Construct/display SVG scenes.
Sam Calisch, 2012
adapted from http://code.activestate.com/recipes/325823-draw-svg-images-in-python/
"""
...
class Paper(object):
    ...
    def add(self,item,orient):
        if orient == 'mountain':
            self.mountains.append(item)
        elif orient = 'valley':
            self.valleys.append(item)
...
def test():
    ...
    paper.add(Circle((300,200),30,(255,0,0)))
    scene.write_svg()
    scene.display()
    return
